...
#[post("/encode")]
async fn encode_handler(body: String, adapter: web::Data<JsonAlBhedTransferAdapter>) -> HttpResponse {
  match adapter.encode(&body) {
    Ok(response) => HttpResponse::Ok().content_type("application/json").body(response),
    Err(error) => HttpResponse::BadRequest().body(error)
  }
}

#[post("/decode")]
async fn decode_handler(body: String, adapter: web::Data<JsonAlBhedTransferAdapter>) -> HttpResponse {
